...
const AnimatedImpact = () => {
  ...
  }, []) // The effect runs once on mount to set up the observer [^1]
        ...
        {cardsData.map((card, i) => (
          <div key={i} className="flex flex-col items-center">
            <div className="w-20 sm:w-24 h-20 sm:h-24 rounded-full bg-[#1C6248] flex items-center justify-center mb-4 shadow-md">
              <img src={card.image || "/placeholder.svg"} alt={card.title} className="w-8 sm:w-10 h-8 sm:h-10" />
            </div>
